    ...
    <div className="space-y-10 max-w-[1440px] mx-auto pb-16 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/[0.06] pb-5 font-mono">
        <div>
          <div className="flex items-center gap-2 mb-1.5 text-xs">
            ...
            <span className="text-[#64748B]">CROSS-VENDOR NORMALIZATION</span>
          </div>
        ...
        <div className="flex items-center gap-3 self-start md:self-auto text-xs">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-md bg-[#00C896] hover:bg-[#00B383] text-[#050709] font-bold transition-all shadow-sm active:scale-[0.98] disabled:opacity-50"
          >
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="p-6 rounded-2xl bg-[#070A10] border border-white/[0.08] space-y-5 font-mono">
            <div className="space-y-2">
              <div className="text-[11px] font-bold text-[#00D9FF] tracking-wider uppercase">
            ...
            <div className="space-y-2 pt-2 border-t border-white/[0.06]">
              <div className="text-[10px] text-[#64748B] uppercase font-semibold">SELECT VENDOR TO TRACE</div>
              <div className="grid grid-cols-3 gap-2 text-xs">
                <button
                  ...
                  className={cn(
                    ...
                    selectedVendor === "cisco"
                      ? "bg-[#00D9FF]/15 border-[#00D9FF] text-[#00D9FF] shadow-[0_0_12px_rgba(0,217,255,0.25)]"
                      : "bg-[#0B0F19] border-white/[0.06] text-[#94A3B8] hover:text-white hover:border-white/[0.15]"
                  )}
                >
                  <div>CISCO IOS</div>
                  <div className="text-[10px] text-[#64748B] font-normal mt-0.5">CLI Native</div>
                </button>
                ...
                >
                  <div>JUNIPER</div>
                  <div className="text-[10px] text-[#64748B] font-normal mt-0.5">Set Hierarchy</div>
                </button>
                ...
                >
                  <div>FORTINET</div>
                  <div className="text-[10px] text-[#64748B] font-normal mt-0.5">Config Tree</div>
                </button>
              </div>
            </div>
            ...
            {activeVendorData ? (
              <div className="p-4 rounded-xl bg-[#0B0F19] border border-white/[0.08] space-y-3.5 animate-fadeIn">
              ...
              <div className="p-4 rounded-xl bg-[#0B0F19] border border-white/[0.04] text-center space-y-2">
                <Compass className="w-5 h-5 text-[#00D9FF] mx-auto" />
                <div className="text-xs font-bold text-[#F8FAFC]">INTERACTIVE TERRAIN INSPECTION</div>
      ...
      <div className="space-y-4 font-mono">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/[0.06] pb-3">
          <div>
            <div className="text-xs font-bold text-[#00D9FF] uppercase tracking-wider">CROSS-OS EQUIVALENCE MATRIX</div>
            <h2 className="text-lg sm:text-xl font-bold font-sans text-[#F8FAFC] mt-0.5">
              Vendor Syntax vs Universal Security Model
            </h2>
          </div>
          <span className="text-[11px] text-[#64748B]">Deterministic Verification</span>
        </div>

        {/* Comparison Table */}
        <div className="overflow-x-auto rounded-2xl border border-white/[0.08] bg-[#070A10]">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-white/[0.08] bg-[#0B0F19] text-[#94A3B8]">
                <th className="p-4 font-bold">SECURITY PROPERTY</th>
                <th className="p-4 font-bold text-[#00D9FF]">CISCO IOS</th>
            ...
            <tbody className="divide-y divide-white/[0.04]">
              {proofState?.comparison_matrix?.map((row, idx) => (
                <tr key={idx} className="hover:bg-[#0B0F19]/60 transition-colors">
                  {/* Property Key & Display */}
                  <td className="p-4 space-y-1">
                    <div className="font-bold text-[#F8FAFC]">{row.display_name}</div>
                    <div className="text-[10px] text-[#00D9FF]">{row.property_key}</div>
                    <div className="text-[9px] text-[#64748B]">{row.target_standard}</div>
                  </td>

                  {/* Cisco */}
                  <td className="p-4 space-y-1">
                    <code className="px-2 py-1 rounded bg-[#03060A] border border-white/[0.06] text-[#00D9FF] text-[11px] block">
                      {row.cisco?.syntax || "Not configured"}
                    </code>
                    {row.cisco?.line && (
                      <div className="text-[10px] text-[#64748B]">[Evidence: Line {row.cisco.line}]</div>
                    )}
                  </td>

                  {/* Juniper */}
                  <td className="p-4 space-y-1">
                    <code className="px-2 py-1 rounded bg-[#03060A] border border-white/[0.06] text-[#10B981] text-[11px] block">
                      {row.juniper?.syntax || "Not configured"}
                    </code>
                    {row.juniper?.line && (
                      <div className="text-[10px] text-[#64748B]">[Evidence: Line {row.juniper.line}]</div>
                    )}
                  </td>

                  {/* Fortinet */}
                  <td className="p-4 space-y-1">
                    <code className="px-2 py-1 rounded bg-[#03060A] border border-white/[0.06] text-[#F59E0B] text-[11px] block">
                      {row.fortinet?.syntax || "Not configured"}
                    </code>
                    {row.fortinet?.line && (
                      <div className="text-[10px] text-[#64748B]">[Evidence: Line {row.fortinet.line}]</div>
                    )}
                  </td>
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-[#070A10] border border-white/[0.08] space-y-6 font-mono">
        <div className="text-center space-y-1 max-w-2xl mx-auto">
          <div className="text-[11px] font-bold text-[#00D9FF] tracking-wider uppercase">END-TO-END PIPELINE</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 text-xs">
          {[
            ...
            <div
              key={idx}
              className="p-3.5 rounded-xl bg-[#0B0F19] border border-white/[0.06] space-y-2 flex flex-col justify-between"
            >
              <div>
                <div className="text-[10px] text-[#00D9FF] font-bold">{item.step}</div>
                <div className="text-xs font-bold text-[#F8FAFC] mt-1">{item.label}</div>
              </div>
              <div className="text-[10px] text-[#94A3B8] font-sans">{item.desc}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-gradient-to-r from-[#070A10] via-[#0B0F19] to-[#070A10] border border-[#00D9FF]/25 space-y-6 font-mono">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/[0.08] pb-4">
          <div className="space-y-1">
            ...
            <h3 className="text-lg sm:text-xl font-bold font-sans text-[#F8FAFC]">
              AI Can Explain. AI Cannot Change The Verdict.
            </h3>
          </div>

          <div className="px-3.5 py-1.5 rounded-md bg-[#070A10] border border-white/[0.08] text-[11px] text-[#10B981] font-bold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-5 rounded-xl bg-[#070A10] border border-emerald-500/30 space-y-3">
            ...
            <ul className="space-y-2 text-xs text-[#E2E8F0] font-sans">
          ...
          {/* Right: AI Advisory */}
          <div className="p-5 rounded-xl bg-[#070A10] border border-cyan-500/30 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-[#00D9FF] uppercase">AI ADVISORY ASSISTANT</span>
              <span className="px-2 py-0.5 rounded text-[10px] bg-[#00D9FF]/15 text-[#00D9FF] font-bold">
                READ-ONLY ADVISORY
              </span>
            </div>
            <ul className="space-y-2 text-xs text-[#E2E8F0] font-sans">
              <li className="flex items-start gap-2">
                <Bot className="w-4 h-4 text-[#00D9FF] shrink-0 mt-0.5" />
