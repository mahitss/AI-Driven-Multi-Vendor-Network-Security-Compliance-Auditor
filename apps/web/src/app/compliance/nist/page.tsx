    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight flex items-center gap-2.5">
            ...
            <span>NIST SP 800-53 Rev. 5 Compliance Catalog</span>
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            National Institute of Standards and Technology security control families evaluated against network configuration baselines.
          </p>
        </div>

        <Link
          href="/audits"
          className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold font-mono self-start sm:self-auto transition-colors"
        >
      ...
      {isLoading ? (
        <div className="py-16 text-center text-slate-400 font-mono text-xs flex items-center justify-center gap-2">
          ...
          <span>Loading NIST SP 800-53 control catalog...</span>
        </div>
      ) : (
          ...
          {controls.map((ctrl) => (
            <div key={ctrl.rule_id} className="p-4 rounded-xl bg-slate-900/60 border border-white/5 space-y-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-indigo-950 text-indigo-300 border border-indigo-800/40 font-bold">
                    {ctrl.control_id}
                  </span>
