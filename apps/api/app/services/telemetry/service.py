...
class TelemetryAggregationService:
    ...
    async def get_complete_telemetry(db: AsyncSession) -> Dict[str, Any]:
        ...
        Guarantees 100% data reconciliation across all visual components.
        """
        ...
        latest_audits = list(latest_audits_res.scalars().all())
        latest_audit_ids = [a.id for a in latest_audits]
        ...
        for a in all_audits:
            cfg = all_configs.get(a.configuration_id)
            device_name = cfg.original_filename if cfg else (a.device_id or f"Audit #{a.id[:8]}")
            ...
            f_stats_stmt = select(
                ...
                func.sum(case((Finding.status == "PASS", 1), else_=0)).label("passed"),
            ...
            total_f = f_stats.total or 0
            open_f = f_stats.open or 0
            pass_f = f_stats.passed or 0
            crit_f = f_stats.critical or 0
            high_f = f_stats.high or 0

            # Calculate audit risk score
            audit_risks = risks_by_audit.get(a.id, [])
            if audit_risks:
                avg_risk = round(sum(r.risk_score for r in audit_risks) / len(audit_risks), 1)
            else:
            ...
            if isinstance(a.summary_stats, dict) and "framework_scores" in a.summary_stats:
                for fw, fw_data in a.summary_stats["framework_scores"].items():
                    if isinstance(fw_data, dict) and "score" in fw_data:
                        fw_scores[fw.upper()] = float(fw_data["score"])

            ts = a.completed_at or a.created_at
            ts_iso = ts.isoformat() if ts else datetime.now(timezone.utc).isoformat()
            ...
            audit_trends.append({
                ...
                "vendor": vendor,
                "timestamp": ts_iso,
                "compliance_score": round(a.score, 1) if a.score is not None else 0.0,
                "risk_score": avg_risk,
        ...
        if latest_audit_ids:
            ...
            active_findings_stmt = (
                ...
                .where(Finding.status.in_(["FAIL", "PARTIAL"]))
                .group_by(Finding.severity)
            )
            sev_res = (await db.execute(active_findings_stmt)).all()
            sev_map = {row[0].upper(): row[1] for row in sev_res}
            ...
            fw_stats_stmt = (
                select(
                    Finding.framework,
                    func.sum(case((Finding.status.in_(["FAIL", "PARTIAL"]), 1), else_=0)).label("failed"),
                    func.sum(case((Finding.status == "PASS", 1), else_=0)).label("passed"),
                    func.count(Finding.id).label("total")
                )
                .where(Finding.audit_id.in_(latest_audit_ids))
            ...
            fw_map = {row[0].upper(): {"failed": row[1] or 0, "passed": row[2] or 0, "total": row[3] or 0} for row in fw_res}
            ...
            for a in latest_audits:
                ...
                profile = cfg.normalized_profile or {}
                ident = profile.get("identity", {})
                hostname = (
                    ident.get("hostname", {}).get("value")
                    if isinstance(ident.get("hostname"), dict)
                    else cfg.original_filename.replace(".cfg", "").replace(".conf", "")
                ) or cfg.original_filename

                vendor = cfg.detected_vendor.lower()
                platform = cfg.detected_platform or (profile.get("platform") or "Enterprise OS")
                ...
                audit_risks = risks_by_audit.get(a.id, [])
                if audit_risks:
                    r_score = round(sum(r.risk_score for r in audit_risks) / len(audit_risks), 1)
                else:
                ...
                for fw in ["CIS", "NIST", "STIG", "ISO"]:
                    ...
                    t_fw = f_fw + p_fw
                    s_fw = round((p_fw / t_fw * 100), 1) if t_fw > 0 else (round(a.score, 1) if a.score else 0.0)
                ...
                heatmap_matrix.append({
                    ...
                    "frameworks": fw_asset,
                    "total_open": open_a,
                    "overall_score": round(a.score, 1) if a.score is not None else 0.0,
                    "risk_score": r_score,
                })
                ...
                # Topology Node
                dev_status = "HARDENED" if (a.score or 0) >= 80 else ("NEEDS_ATTENTION" if (a.score or 0) >= 60 else "HIGH_RISK")
                topology_nodes.append({
                    ...
                    "critical_findings": crit_a,
                    "status": dev_status,
                    "last_seen": (cfg.uploaded_at or a.created_at).isoformat(),
                })
        ...
        total_rem = rem_res.total or 0
        avail_rem = rem_res.available or 0
        rev_rem = rem_res.reviewed or 0

        # Query applied & verified from agent memory manager
        agent_sessions = await AgentMemoryManager.list_recent_sessions(limit=20)
        applied_rem = sum(s.final_report.remediations_applied for s in agent_sessions if s.final_report)
        verified_rem = sum(s.final_report.remediations_applied for s in agent_sessions if s.final_report and s.status == "COMPLETED")
