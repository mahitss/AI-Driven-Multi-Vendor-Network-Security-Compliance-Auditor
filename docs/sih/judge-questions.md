# NetVigil — Evaluator & Jury Q&A Technical Defense Guide

**Problem Statement:** SIH26155 (NTRO)  
**Document Purpose:** In-depth technical justifications, limitation disclosures, and architectural defenses for jury questions.
...
### Q8: What is NetVigil's biggest limitation today?
**Answer (Honest & Technical):**
1. **Initial Vendor Depth**: Currently verified across Cisco IOS, Juniper JunOS, and Fortinet FortiOS. Additional vendors (Palo Alto PAN-OS, Arista EOS, Check Point) currently route through Adaptive Training rather than dedicated native AST parsers.
2. **Remediation Automation**: Deliberately requires manual operator review and copy/download rather than automated remote push to eliminate production network outage risks.
3. **Scalability Path**: The system scales linearly by distributing stateless FastAPI parsing workers behind a load balancer and utilizing PostgreSQL read-replicas, achieving over 1,000 device audits per minute.
